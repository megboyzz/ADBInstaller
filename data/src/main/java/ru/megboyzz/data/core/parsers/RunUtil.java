...
import static ru.megboyzz.data.core.parsers.StreamFunKt.copyStreams;
...
public class RunUtil implements IRunUtil {
    ...
    public CommandResult runTimedCmdWithInput(final long timeout, String input,
        ...
        return runTimedCmdWithInput(timeout, input, List.of(command));
    }
    /**
     * {@inheritDoc}
     */
    ...
    public Process runCmdInBackground(final String... command) throws IOException  {
        ...
        System.out.printf("Running %s", fullCmd);
    ...
    public Process runCmdInBackground(final List<String> command) throws IOException  {
        System.out.printf("Running %s", command);
        return createProcessBuilder(command).start();
    }
    /**
     * {@inheritDoc}
    ...
    public CommandStatus runTimed(long timeout, IRunUtil.IRunnableResult runnable,
        ...
        System.out.printf("Running command with timeout: %dms", timeout);
        runThread.start();
        ...
        do {
            try {
                ...
                if (mIsInterruptAllowed.get()) {
                    System.out.printf("runTimed: interrupted while joining the runnable");
                    break;
                }
                else {
                    System.out.printf("runTimed: received an interrupt but uninterruptible mode, ignoring");
                }
            }
        ...
        if (runThread.getStatus() == CommandStatus.TIMED_OUT
            ...
            System.out.printf("runTimed: Calling interrupt, status is %s", runThread.getStatus());
            runThread.interrupt();
        }
        checkInterrupted();
    ...
    public boolean runTimedRetry(long opTimeout, long pollInterval, int attempts,
        ...
        for (int i = 0; i < attempts; i++) {
            ...
            System.out.printf("operation failed, waiting for %d ms", pollInterval);
            sleep(pollInterval);
        }
        return false;
    }
    ...
    public boolean runEscalatingTimedRetry(final long opTimeout,
        ...
        while (true) {
            ...
            if (remainingTime <= 0) {
                System.out.printf("operation is still failing after retrying for %d ms", maxTime);
                return false;
    ...
    public void allowInterrupt(boolean allow) {
        System.out.printf("run interrupt allowed: %s", allow);
    ...
    private static class RunnableNotifier extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                System.out.printf("runutil interrupted");
                status = CommandStatus.EXCEPTION;
            } catch (Exception e) {
                if (mLogErrors) {
                    System.out.printf("Exception occurred when executing runnable");
    ...
    private class RunnableResult implements IRunUtil.IRunnableResult {
        ...
        public boolean run() throws Exception {
            ...
            System.out.printf("Running %s", mProcessBuilder.command());
            ...
            if (rc == 0) {
                ...
                System.out.printf("%s command failed. return code %d", mProcessBuilder.command(), rc);
            }
            return false;
        }
        @Override
        public void cancel() {
            if (mProcess != null) {
                System.out.printf("Cancelling the process execution");
                mProcess.destroy();
                mProcess = null;
                try {
                    ...
                    if (!mCountDown.await(PROCESS_DESTROY_TIMEOUT_SEC, TimeUnit.SECONDS)) {
                        System.out.printf("Process still not terminated, interrupting the execution thread");
                        mExecutionThread.interrupt();
                        mCountDown.await();
                    }
                } catch (InterruptedException e) {
                    System.out.printf("interrupted while waiting for process output to be saved");
                }
            }
        }
    }
    ...
    private static Thread inheritIO(final InputStream src, final OutputStream dest) {
        Thread t = new Thread(() -> copyStreams(src, dest));
        t.start();
        return t;
    }
}